...
This section lists some of the common issues encountered when using Workbench for Zephyr, along with possible solutions and workarounds.

---

## Conflict with STM32 VS Code Extension

When the **STM32Cube for Visual Studio Code** is installed alongside Workbench for Zephyr, it can cause conflicts that break the build process.  
...
- Alternatively, install **Workbench for Zephyr** in a [standalone portable VS Code](https://code.visualstudio.com/docs/editor/portable).
...
When running Workbench for Zephyr import workspace or build, you may encounter the following error message:
...
Make sure your **workspace folder** and the **application folders** used by Workbench for Zephyr are excluded from Windows Defender (Microsoft Defender) real-time scanning.
