...
To display the application ECLAIR Manager, right-click on the application Analysis > Static Code Analysis > ECLAIR Manager or use the `ECLAIR Manager` command in the Command Palette.
...
- **ECLAIR** detection **status and management**
...
> 🛈 The configurations can be saved with the "Save" button. They will be store in the `.vscode/zephyr-workbench.eclair.json` file of each workspace. You can safely track this file in your versioning system.
...
Preset repositories are normal git repository that contains special `.ecl` files, which are recognized and showed in the UI.

The reference repository for preset is the one provided by Bugseng[^bugseng-preset-repo], which is available by default and contains, along with a variety of curated presets, the specification of the preset format.

![alt text](/img/zw/static-code-analysis/eclair-manager/repo-manager.png)
...
The report is displayed in the Terminal. For more information, please refer to:
https://docs.zephyrproject.org/latest/develop/sca/eclair.html

#### Selecting a preset

The ECLAIR Manager classifies presets in 3 categories:
...
For each kind of preset you will be able to select one or more presets to apply for the analysis from the available presets in the repositories, or choosing a local file.
...
Independently of the kind of configuration you choose, you can also select an additional ECL file to fine-tune the configuration for the analysis. This file will be merged with the main configuration selected, allowing you to easily customize the analysis without having to create a whole new preset for that.
